...
export default function App() {
  ...
  const clearPersonSearch = () => {
    setSearchPerson(null);
    if (!searchTitle && selectedTags.length === 0 && !isFilterApplied && searchCollection === null) {
      resetSearch();
    } else {
      setApiPage(1); setNextApiPage(1); setForceSearch(true);
    }
  };

  const clearCollectionSearch = () => {
    setSearchCollection(null);
    if (!searchTitle && selectedTags.length === 0 && !isFilterApplied && searchPerson === null) {
      resetSearch();
    } else {
      setApiPage(1); setNextApiPage(1); setForceSearch(true);
    }
  ...
  const renderAppHeader = () => (
    <header className="sticky top-0 z-40 bg-[#141414]/95 backdrop-blur-md px-4 py-3 flex items-center justify-between border-b border-zinc-800/80 shrink-0">
  ...
  const renderDetailModal = () => {
    ...
    return (
      ...
      <div className={`absolute inset-0 z-[60] flex flex-col bg-[#141414] ${swipeOutTarget === 'detail' ? 'slide-out-right' : 'animate-in slide-in-from-bottom-10 fade-in duration-300'}`}
        ...
        {currentModalMode === 'delete_confirm' && (
          <div className="absolute inset-0 bg-black/80 backdrop-blur-md z-[80] flex items-center justify-center p-6 pointer-events-auto">
        ...
        {/* ↓中身のレイアウト（背景指定を親に移したのでここは透明でOK） */}
        <div className="flex-1 flex flex-col overflow-hidden relative pointer-events-auto">
  ...
  const renderReviewModal = () => {
    ...
    return (
      <div className="absolute inset-0 bg-[#141414] z-[70] flex flex-col animate-in slide-in-from-bottom-10 fade-in duration-300">
            ...
            <div className="flex gap-2 relative">
              <div className="relative flex-1">
                ...
                <input 
                  type="text" 
                  value={searchTitle} 
                  onChange={(e) => { setSearchTitle(e.target.value); setApiPage(1); setNextApiPage(1); setForceSearch(false); setSearchPerson(null); setSearchCollection(null); }} 
                  placeholder="映画を検索..." 
                ...
                />
                {searchTitle && <button onClick={() => { setSearchTitle(''); setApiPage(1); setNextApiPage(1); setForceSearch(false); }} className="absolute right-3 top-1/2 -translate-y-1/2 text-zinc-400 cursor-pointer"><X size={16} /></button>}
              </div>
              <button onClick={() => {
                if (!showFilters) {
            ...
            {showFilters && (
              <div ref={filterMenuRef} className="absolute top-full right-0 left-0 mt-2 bg-zinc-900 p-4 rounded-xl border border-zinc-700 shadow-2xl z-50 animate-in fade-in">
                ...
                <div className="flex gap-3 mt-6 pt-4 border-t border-zinc-800">
                  <button onClick={() => {
                    setTempFilters(defaultFilters); setAppliedFilters(defaultFilters); setShowFilters(false); setApiPage(1); setNextApiPage(1); setForceSearch(false);
    ...
    <div className="bg-[#141414] h-[100dvh] w-full flex justify-center font-sans selection:bg-red-900/30 text-zinc-200 overflow-hidden app-wrapper">
      ...
      <div className="w-full max-w-md h-full bg-[#141414] shadow-2xl relative flex flex-col overflow-hidden">
        ...
        <div className="flex-1 relative overflow-hidden flex flex-col min-h-0">
          <div style={{ display: (activeTab === 'home' || activeTab === 'genre_view') ? 'flex' : 'none' }} className="w-full h-full flex-col absolute inset-0 z-0">{renderHome()}</div>
          <div style={{ display: activeTab === 'genre_view' ? 'flex' : 'none' }} className="w-full h-full flex-col absolute inset-0 z-10">{renderGenreView()}</div>
          <div style={{ display: activeTab === 'watchlist' ? 'flex' : 'none' }} className="w-full h-full flex-col absolute inset-0 z-0">{renderMyList('watchlist')}</div>
          <div style={{ display: activeTab === 'watched' ? 'flex' : 'none' }} className="w-full h-full flex-col absolute inset-0 z-0">{renderMyList('watched')}</div>
